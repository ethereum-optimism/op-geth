...
func TestCalcBaseFeeMinBaseFee(t *testing.T) {
	...
	tests := []struct {
		...
		// 2e9 + 40_000_000 = 2_040_000_000, which is greater than 2^20
		{2e9, parentGasLimit/elasticity + 10_000_000, 2_040_000_000, uint8(1)<<4 | uint8(9)},
		...
		// 2_097_152 - 4194.304 = 2_092_957.696, which is greater than 2e6
		{2_097_152, parentGasLimit/elasticity - 1_000_000, 2_092_958, uint8(2)<<4 | uint8(6)},
	}
	for i, test := range tests {
		parent := &types.Header{
			Number:   common.Big32,
			...
			BaseFee:  big.NewInt(test.parentBaseFee),
			Time:     14,
			Extra:    EncodeMinBaseFeeExtraData(denom, elasticity, test.minBaseFeeFactors),
		}
		if have, want := CalcBaseFee(opConfig(), parent, parent.Time+2), big.NewInt(test.expectedBaseFee); have.Cmp(want) != 0 {
			t.Errorf("test %d: have %d  want %d, ", i, have, want)
		}
