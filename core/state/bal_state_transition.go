...
func (s *BALStateTransition) GetStorageRoot(addr common.Address) common.Hash {
	// Check post-states first (modified accounts after IntermediateRoot)
	if acct, ok := s.postStates[addr]; ok {
		return acct.Root
	}
	// Check prestates for unmodified accounts
	if p, ok := s.prestates.Load(addr); ok {
		return p.(*types.StateAccount).Root
	}
	// Account not touched in this block, read from the state trie
