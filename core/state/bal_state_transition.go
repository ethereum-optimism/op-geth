...
// GetStorageRoot returns the storage root of the given address.
// It first checks post-state accounts (computed during IntermediateRoot),
// then falls back to reading the account from the state trie.
func (s *BALStateTransition) GetStorageRoot(addr common.Address) common.Hash {
	if acct, ok := s.postStates[addr]; ok {
		return acct.Root
	}
	// Account was not modified in this block; read it from the state trie.
	enc, err := s.stateTrie.GetAccount(addr)
	if err != nil || enc == nil {
		return common.Hash{}
	}
	return enc.Root
