...
func (st *stateTransition) buyGas() error {
	...
	if _, overflow := mgval.MulOverflow(mgval, st.msg.GasPrice); overflow {
	...
	if !st.msg.SkipNonceChecks && !st.msg.SkipTransactionChecks {
		if st.evm.Context.L1CostFunc != nil {
			if l1CostBig := st.evm.Context.L1CostFunc(st.msg.RollupCostData, st.evm.Context.Time); l1CostBig != nil {
				var overflow bool
				l1Cost, overflow = uint256.FromBig(l1CostBig)
				if overflow {
					return fmt.Errorf("%w: address %v l1Cost exceeds 256 bits", ErrInsufficientFunds, st.msg.From.Hex())
				}
	...
	if st.msg.GasFeeCap != nil {
		...
		if l1Cost != nil {
			if _, overflow := balanceCheck.AddOverflow(balanceCheck, l1Cost); overflow {
				return fmt.Errorf("%w: address %v required balance exceeds 256 bits", ErrInsufficientFunds, st.msg.From.Hex())
			}
		}
		if operatorCost != nil {
