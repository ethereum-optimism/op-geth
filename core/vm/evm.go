...
func (ctx PrecompileContextImpl) GetL1ArchiveRpc() *string {
	return ctx.L1ArchiveNodeRPC
}

func (ctx PrecompileContextImpl) GetState(addr common.Address, slot common.Hash) common.Hash {
	return ctx.stateDb.GetState(addr, slot)
}

func (evm *EVM) getPrecompileInfo() PrecompileContext {
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if isPrecompile {
		ret, gas, err = RunPrecompiledContract(evm.getPrecompileInfo(), p, input, gas)
	} else {
