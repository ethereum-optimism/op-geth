...
func (evm *EVM) Call(caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
    ...
    if isPrecompile {
        ret, gas, err = RunPrecompiledContractWithState(p, input, gas, evm.Config.Tracer, evm.StateDB, false)
    } else {
...
func (evm *EVM) DelegateCall(originCaller common.Address, caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// It is allowed to call precompiles, even via delegatecall
    if p, isPrecompile := evm.precompile(addr); isPrecompile {
        ret, gas, err = RunPrecompiledContractWithState(p, input, gas, evm.Config.Tracer, evm.StateDB, true)
    } else {
...
func (evm *EVM) StaticCall(caller common.Address, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
    ...
    if p, isPrecompile := evm.precompile(addr); isPrecompile {
        // In DELEGATECALL, msg.sender is the original caller of the current context.
        ret, gas, err = RunPrecompiledContractWithState(p, input, gas, evm.Config.Tracer, evm.StateDB, false)
    } else {
		// Initialise a new contract and set the code that is to be used by the EVM.
		// The contract is a scoped environment for this execution context only.
