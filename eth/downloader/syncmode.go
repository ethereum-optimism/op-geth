...
func newSyncModer(mode ethconfig.SyncMode, chain BlockChain, disk ethdb.KeyValueReader) *syncModer {
	if mode == ethconfig.FullSync {
		...
		// OP Stack: permit snap sync if the chain head is the Bedrock block.
		if head.Number.Uint64() > 0 && chain.HasState(head.Root) && (!chain.Config().IsOptimism() || head.Number.Cmp(chain.Config().BedrockBlock) != 0) {
