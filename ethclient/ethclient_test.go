...
func newTestBackend(t *testing.T, enableHistoricalState bool) (*node.Node, []*types.Block) {
	...
	if enableHistoricalState {
		g = genesisForHistorical
	}
	config := &ethconfig.Config{Genesis: g}
...
func generateTestChain(enableHistoricalState bool) []*types.Block {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(g, ethash.NewFaker(), 2, generate)
	return append([]*types.Block{g.ToBlock()}, blocks...)
}
