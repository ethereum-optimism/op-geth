...
func (miner *Miner) makeEnv(parent *types.Header, header *types.Header, coinbase common.Address, witness bool, rpcCtx context.Context) (*environment, error) {
	...
	if miner.chainConfig.Optimism != nil { // Allow the miner to reorg its own chain arbitrarily deep
		if historicalBackend, ok := miner.backend.(BackendWithHistoricalState); ok {
			...
			sdb, release, err = historicalBackend.StateAtBlock(context.Background(), parentBlock, ^uint64(0), nil, false, false)
			if err != nil {
				return nil, err
			}
			sdb = sdb.Copy()
	...
	return &environment{
		...
		header:   header,
		witness:  sdb.Witness(),
		evm:      vm.NewEVM(core.NewEVMBlockContext(header, miner.chain, &coinbase, miner.chainConfig, sdb), hookedState, miner.chainConfig, vmConfig),
		alTracer: alTracer,
		rpcCtx:   rpcCtx,
	}, nil
}
