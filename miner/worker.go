...
type generateParams struct {
	...
	noTxPool   bool               // Flag whether additional transactions can be taken out of the tx pool
}
...
func (w *worker) generateWork(genParams *generateParams) (*types.Block, error) {
	...
	// force-txs done, fill rest of block with transactions from the tx-pool, if it is enabled
	if !genParams.noTxPool {
...
func (w *worker) getSealingBlock(parent common.Hash, timestamp uint64, coinbase common.Address, random common.Hash, forceTxs types.Transactions, noTxPool bool) (*types.Block, error) {
	req := &getWorkReq{
		params: &generateParams{
			timestamp:  timestamp,
			forceTime:  true,
