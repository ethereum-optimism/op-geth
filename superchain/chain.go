...
// commitOnce caches the result of reading the COMMIT entry from the embedded
// superchain-configs.zip. The bundle is generated by sync-superchain.sh, which
// always writes a COMMIT entry, so any error here means the bundle is corrupt
// and we should fail loud rather than silently.
var commitOnce = sync.OnceValue(func() string {
	...
	if err != nil {
		panic(fmt.Errorf("opening embedded superchain-configs.zip: %w", err))
	}
	f, err := zr.Open("COMMIT")
	if err != nil {
		panic(fmt.Errorf("reading COMMIT entry from embedded zip: %w", err))
	}
	defer f.Close()
	raw, err := io.ReadAll(f)
	if err != nil {
		panic(fmt.Errorf("reading COMMIT entry contents: %w", err))
	}
	return string(bytes.TrimSpace(raw))
})

// EmbeddedRegistryCommit returns the superchain-registry commit hash that the
// embedded superchain-configs.zip was generated from. Useful for downstream
// consumers that pin their own snapshot of the registry and want to detect
// drift against op-geth's pinned commit. Panics if the embedded bundle is
// corrupt — the COMMIT entry is always present in correctly-built bundles.
func EmbeddedRegistryCommit() string {
	return commitOnce()
}
